...
// Capture TravelCompanyID from login redirect
if (!isset($_GET['companyID']) || empty($_GET['companyID'])) {
    die("Travel Company ID not found. Please login again.");
}
$companyID = intval($_GET['companyID']);

// Fetch optional charges for display
...
if (isset($_POST['book'])) {
    ...
    // Get final total fee after multiplying by rooms
    $totalFee = isset($_POST['total_fee']) ? floatval($_POST['total_fee']) : 0;

    // Insert into ReservationTravelCompany table
    $stmt = $conn->prepare("
        INSERT INTO ReservationTravelCompany 
        (CheckIn, CheckOut, NumGuests, NumRooms, Status, BedCount, TravelCompanyID, Total_Fee) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$checkIn, $checkOut, $numGuests, $numRooms, $status, $bedCount, $companyID, $totalFee]);
    ...
    $successMsg = "Reservation successfully created for the travel company.";
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Travel Company Reservation</title>
...
<section class="pt-32 pb-12 text-center text-white relative">
  <h1 class="text-4xl md:text-6xl font-bold drop-shadow-lg mb-4 animate-fadeIn">Travel Company Room Reservation</h1>
  <p class="text-lg md:text-xl max-w-2xl mx-auto opacity-90 animate-slideUp">
    Book rooms for your travel groups with ease.
  </p>
</section>
...
<section class="max-w-4xl mx-auto px-6 py-12 bg-white/80 rounded-3xl shadow-2xl backdrop-blur-lg">
  ...
  <?php if (!empty($successMsg)) : ?>
    <div class="mb-6 p-4 bg-green-500 text-white rounded-xl text-center font-semibold shadow-lg">
      <?= $successMsg ?>
    </div>
  <?php endif; ?>
  ...
  <form method="POST" id="reservationForm">
    ...
    <div class="grid md:grid-cols-3 gap-6 mb-6">
      ...
      <div class="flex flex-col">
        <label class="mb-2 font-semibold text-[#124170]" for="numrooms">Number of Rooms</label>
        <input type="number" name="numrooms" id="numrooms" min="1" value="1" required
          class="rounded-xl border border-gray-300 p-3 focus:ring-2 focus:ring-[#67C090] transition">
      </div>
    </div>
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <?php foreach ($optionalCharges as $charge): ?>
          <label class="flex items-center space-x-3 p-3 bg-gray-100 rounded-xl hover:shadow-lg transition cursor-pointer">
            <input type="checkbox" name="optional_charges[]" 
              value="<?php echo $charge['ChargeID']; ?>" 
              data-amount="<?php echo $charge['Amount']; ?>" 
              class="form-checkbox h-5 w-5 text-[#67C090]">
    ...
    <div class="mb-6 flex items-center justify-between">
      <button type="button" id="calculateFee" class="bg-[#26667F] text-white px-6 py-3 rounded-xl font-semibold shadow-md hover:bg-[#67C090] transition">
        Calculate Fee
      </button>
...
<footer class="bg-[#124170] text-white text-center py-6 mt-12">
  <p class="mb-2">© <?= date("Y"); ?> Crystal Heaven. All rights reserved.</p>
</footer>

<script src="../js/travelcompany_booking.js"></script>
</body>
</html>
