...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // 🔹 Query Manager table instead of Admin
        $stmt = $conn->prepare("SELECT ManagerID, Password FROM Manager WHERE Username = :username LIMIT 1");
        ...
        if ($stmt->rowCount() > 0) {
            ...
            if (password_verify($password, $manager['Password'])) {
                ...
                header("Location: manager_dashboard.php?managerID=" . $manager['ManagerID']);
                exit();
            } else {
...
<head>
    <meta charset="UTF-8">
    <title>Manager Login | Hotel Reservation System</title>
...
<body class="flex flex-col min-h-screen">
    ...
    <nav class="bg-transparent fixed top-0 w-full z-50">
        <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
            <h1 class="text-2xl font-bold text-white tracking-wide">Hotel Manager</h1>
            <div class="space-x-6">
    ...
    <main class="flex-grow flex items-center justify-center px-6">
        <div class="glass-card shadow-2xl rounded-2xl p-10 max-w-md w-full animate__animated" id="login-card">
            <h2 class="text-3xl font-bold text-center text-white mb-6">Manager Login</h2>
            ...
            <form method="POST" action="manager_login.php" class="space-y-5">
                <!-- Username -->
                <div>
            ...
            <div class="mt-6 text-center text-white">
                <p>Still not a manager? <a href="manager_register.php" class="underline hover:text-yellow-300 transition">Register</a></p>
            </div>
        </div>
    </main>
    ...
    <!-- External JS -->
    <script src="../js/Manager_Login.js" defer></script>
</body>
</html>
